...
vacant_gdf.to_file("vacant_gdf.shp")
...
# Plot the city tracts
dissolved.plot(ax=ax, column='average_income', legend=True, edgecolor='black').set_axis_off()
...
# Plot the city tracts
dissolved.plot(ax=ax, column='average_price', legend=True, edgecolor='black').set_axis_off()
...
```{python}
#convert vacancy dataframe using code from the app
```

```{python}
#1) install required packages: folium; this is in the requirements.txt
#2 shiny run --reload C:\Users\EM\Documents\GitHub\DAPII_final\vacancy_tabs\app.py
