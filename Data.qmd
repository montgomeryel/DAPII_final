...
directory = r"/Users/griffinsharps/Documents/Repos/DAPII_final"
...
state_county_tract_zcta_prices = combined_data.astype({'tract': 'str'}).merge(
    crosswalk_merge, 
    how='left', 
    on='tract'
)
...
output_file = os.path.join(directory, "state_county_tract_zcta_prices.csv")
state_county_tract_zcta_prices.to_csv(output_file, index=False)
```

```{python}
# Column names
metadata_file = os.path.join(directory, "B25075_metadata.json")
...
# Rename columns based on metadata_dict
census_data_complete = state_county_tract_zcta_prices.rename(columns=metadata_dict)

# Replace "!!" in column names with a space
census_data_complete.columns = census_data_complete.columns.str.replace("!!", " ", regex=False)
```
...
```{python}
merged_census_and_geo.columns[2:27]
```
...
```{python}
 def ave_price(row):
    ...
    calc_1 = row['Estimate Total: Less than $10,000'] * ((0+9999)/2)
    calc_2 = row['Estimate Total: $10,000 to $14,999'] * ((10000+14999)/2)
